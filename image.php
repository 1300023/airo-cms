...
if ((!$maxWidth && !$maxHeight) || (!$color && $maxWidth >= $width && $maxHeight >= $height))
{
	...
	header('Last-Modified: ' . gmdate('D, d M Y H:i:s', filemtime($docRoot . $image)) . ' GMT');
...
header("Content-type: $mime");
header('Last-Modified: ' . gmdate('D, d M Y H:i:s', filemtime($resized)) . ' GMT');
header('Content-Length: ' . filesize($resized));
